...
1. Install the UI Bridge `1.4.6`; uninstall an older Workspace Probe from every tested Workspace host or force-install the new Workspace patch.
2. Reload the local UI Bridge host after installing `artifacts/agent-pivot-attention-ui-bridge-1.4.6.vsix`.
...
Do not rebuild and reuse an unchanged probe artifact version. UI Bridge is `1.4.6`; Workspace Probe is `0.0.5`. Installing in a different Profile does not update an extension that remains installed in the Profile under test.
