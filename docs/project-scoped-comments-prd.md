...
Comments 侧边栏
┌──────────────────────────────────────┐
│ + Note        3 项项目 · 2 项本 Session │
├──────────────────────────────────────┤
│ 项目 [vscode-dashboard]  本项目全部 session 可见 │
│ ┌──────────────────────────────────┐ │
│ │ 记一笔：bug、想法、待办…            │ │  ← 常驻速记框
│ │ + 标签                       添加 │ │
│ └──────────────────────────────────┘ │
│ (全部·3) (bug·1) (idea·1) (optimize·1) │  ← tag 过滤 chips
│ ┌──────────────────────────────────┐ │
│ │ [bug ×] [+]  Open      ✈ ✎ 🗑    │ │
│ │ 遥测条在窄窗口下横向溢出…           │ │
│ │ 出处 · codex session              │ │
│ │ ❝ telemetry overflowed at 400px… │ │
│ │ #1 · 2 小时前 · → 已发送至 kimi · 35 分钟前 │ │
│ └──────────────────────────────────┘ │
│ …                                    │
├──────────────────────────────────────┤
│ 本 Session [kimi · 当前 viewer]        │
│ （现有引用批注 / Session note，原样）    │
├──────────────────────────────────────┤
│ 全部 | Open | Done     发送 清除 清空   │
└──────────────────────────────────────┘
```
...
- 项目分区固定在本 Session 分区**上方**。
- 项目分区永远渲染（含空状态）；本 Session 分区维持现有行为。
- 两个分区各自滚动在同一边栏容器内，不引入嵌套滚动。
...
v1 固定按 createdAt 倒序，不支持拖拽排序（P2 候选）。本 Session 分区的拖拽排序行为不变。
...
- **P2（候选）**：多选 session 广播派发、项目笔记纳入全局搜索、拖拽排序。
