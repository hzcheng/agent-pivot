...
- **CI**: `verify.yml` (the Linux gate runs as four parallel shards — linux-core / linux-browser / linux-safety / linux-release — aggregated by the required `quality-linux` check; platform-windows / tmux-smoke-linux are also required), weekly `scheduled-verification.yml`, `release-vsix.yml` (tagged releases also auto-publish to the VS Code Marketplace on minor/major bumps via the `VSCE_PAT` secret, skipping patch-only bumps unless forced; see RELEASE-MARKETPLACE-PUBLISH-001).
