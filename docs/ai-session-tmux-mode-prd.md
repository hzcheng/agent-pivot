...
Agent Pivot 现在可以在 `OPEN PROJECT` 卡片下展示 Codex、Kimi、Claude 等智能体 session。用户点击某个 session 时，插件会打开 VS Code 自带的集成终端，并运行对应 provider 的恢复命令，例如：
...
> 我希望重要的智能体 session 即使在可见的 VS Code terminal 不见了以后，也能继续运行；之后我再回来时，Agent Pivot 可以重新连接到同一个正在运行的 session。

`tmux` 是一个适合第一版落地的方案，因为它可以把长期运行的 shell 进程从可见 terminal 客户端中解耦出来。Agent Pivot 可以把智能体命令启动在一个命名的 tmux session 里，之后再用 VS Code terminal attach 回这个 tmux session。
...
- 不要求所有用户都安装 tmux 才能使用 Agent Pivot。
...
用户在远程 SSH 服务器上工作。他从 Agent Pivot 启动一个 Codex 或 Kimi session，然后关闭 VS Code，稍后再回来。只要远程主机仍然在线，Agent Pivot 就可以重新 attach 到原来的 tmux session。
...
```json
"agentPivot.aiSessionTerminalMode": "vscode"
```
...
```json
"agentPivot.aiSessionTmuxCommand": "tmux"
```
...
如果启用了 `tmux` mode，Agent Pivot 可以在 tooltip 或 terminal name 中弱提示当前使用 tmux，但不在 session card 中增加额外视觉噪音。
...
```text
Controls how Agent Pivot opens AI sessions. "vscode" runs the agent directly in a VS Code terminal. "tmux" runs the agent inside a named tmux session and attaches VS Code terminals to it. Tmux keeps the session alive only while the execution host remains awake and running.
```

中文语义：

```text
控制 Agent Pivot 如何打开 AI session。"vscode" 会直接在 VS Code terminal 中运行智能体。"tmux" 会把智能体运行在命名 tmux session 中，并让 VS Code terminal attach 到它。Tmux 只有在执行宿主保持唤醒并运行时才能保活 session。
```

当启用 tmux mode 但当前环境找不到 tmux 时：

```text
Agent Pivot could not find tmux in this environment. Install tmux, or switch AI Session Terminal Mode back to "vscode".
```

中文语义：

```text
Agent Pivot 在当前环境中找不到 tmux。请安装 tmux，或把 AI Session Terminal Mode 切回 "vscode"。
```
...
```bash
tmux new-session -A -s agent-pivot-codex-019f2c7f "codex resume --cd '/work/project' '019f2c7f-...'"
```
...
```text
agent-pivot-new-<provider>-<timestamp>-<random>
```

流程：

1. 用户点击 create session。
2. Agent Pivot 像现在一样询问本地显示 title。
3. Agent Pivot 用 provider 的 new-session command 启动一个 pending tmux session。
4. 现有 pending-session resolver 继续监听 provider 新生成的 session。
5. 当 Agent Pivot 发现 provider session ID 后，记录这个 session ID 和 tmux session 的关联。
6. 如果安全可行，可以把 pending tmux session rename 成稳定的 provider/session ID 名称。

第一版中 rename 可以是可选项。如果 rename 风险较高，可以保留 pending tmux 名称，只要 Agent Pivot 后续仍能把发现到的 provider session ID 和该 tmux session 关联起来即可。

### Attach 已存在的 Tmux Session

即使 Agent Pivot 没有内存中的 terminal entry，也应该能通过 tmux session name 重新 attach。
...
```text
agent-pivot-<provider>-<short-hash>
```
...
```text
Agent Pivot could not start the tmux session for this AI chat. Check that tmux works in this environment.
```

中文语义：

```text
Agent Pivot 无法为这个 AI chat 启动 tmux session。请检查当前环境中的 tmux 是否可用。
```

完整命令和错误写入 Agent Pivot output channel，但 UI message 中避免暴露敏感信息。
...
```text
The tmux session exists, but Agent Pivot could not attach to it.
```

中文语义：

```text
tmux session 存在，但 Agent Pivot 无法 attach 到它。
```
...
Codex title 仍然作为 Agent Pivot 本地 alias。Agent Pivot 不应该尝试把 title 写入 Codex。
...
如果 cwd 可用，Agent Pivot 应该先进入该 cwd，再运行 Claude，保持当前行为一致。
...
1. 用户在 SSH 项目中打开 Agent Pivot。
2. `aiSessionTerminalMode` 为 `tmux`。
3. 用户点击一个 Codex session。
4. Agent Pivot 打开名为 `Codex tmux: <session>` 的 terminal。
5. terminal attach 到 `agent-pivot-codex-<hash>`。
6. 用户关闭 VS Code。
7. Codex 继续在 SSH host 上运行。
8. 用户重新打开项目并点击同一个 session。
9. Agent Pivot attach 到同一个 tmux session。
...
4. Agent Pivot 弹出 warning，提供本次使用 VS Code terminal 或打开 settings。

## 可观测性

Agent Pivot 应该把 tmux 生命周期事件写入 output channel：
...
1. 当 `agentPivot.aiSessionTerminalMode` 为 `vscode` 时，所有 AI session 行为保持不变。
...
5. reload VS Code 后，Agent Pivot 仍能 attach 到已存在的 tmux session。
...
- 全局 `agentPivot.aiSessionTerminalMode` 设置。
- `vscode` 和 `tmux` 两种模式。
...
关键产品决策是：Agent Pivot 应该如何命名这个能力。
