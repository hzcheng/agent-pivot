...
export interface DashboardMessageHandlers {
    ...
    handoffAiSession?: DashboardAiSessionCreateMessageHandler;
...
export function createDashboardMessageRouter(handlers: DashboardMessageHandlers): (message: unknown) => Promise<void> {
    return async message => {
        ...
        if (messageType === 'handoff-ai-session' && handlers.handoffAiSession) {
            await handlers.handoffAiSession(message);
            return;
        }
