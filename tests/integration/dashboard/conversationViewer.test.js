...
test('WORKTREE-CHANGES-PANEL-001 session switches republish the changes state for the new session', async t => {
    const fixtureA = await worktreeChangesFixture(t);
    const fixtureB = await worktreeChangesFixture(t);
    await fs.promises.writeFile(
        path.join(fixtureB.worktreePath, 'also-changed.ts'), 'y\n');
    const baselineA = {
        commitSha: fixtureA.baselineSha, capturedAt: Date.now(),
        source: { kind: 'branch', fullRef: 'refs/heads/main' },
    };
    const baselineB = {
        commitSha: fixtureB.baselineSha, capturedAt: Date.now(),
        source: { kind: 'branch', fullRef: 'refs/heads/main' },
    };
    const keyOf = async fixture => ({
        repositoryKey: await fs.promises.realpath(
            path.join(fixture.repo, '.git')),
        canonicalWorktreePath: await fs.promises.realpath(
            fixture.worktreePath),
    });
    const keyA = await keyOf(fixtureA);
    const keyB = await keyOf(fixtureB);
    const groupsBySession = {
        'session-a': {
            groupId: 'group-a', primaryMemberId: 'member-a',
            members: [{
                memberId: 'member-a', repositoryKey: keyA.repositoryKey,
                worktreeKey: keyA, branchName: 'agent-pivot/task-a',
                path: keyA.canonicalWorktreePath, state: 'ready',
                baseline: baselineA,
            }],
        },
        'session-b': {
            groupId: 'group-b', primaryMemberId: 'member-b',
            members: [{
                memberId: 'member-b', repositoryKey: keyB.repositoryKey,
                worktreeKey: keyB, branchName: 'agent-pivot/task-b',
                path: keyB.canonicalWorktreePath, state: 'ready',
                baseline: baselineB,
            }],
        },
    };
    ...
    const { viewer, panel } = createViewer({
        changes: {
            resolveSessionIdentity: async target => ({
                worktreeKey: target.sessionId === 'session-b' ? keyB : keyA,
                navigationIdentity: 'nav',
            }),
            resolveWorktreeKey: async () => undefined,
            findGroupByWorktreeKey: (_nav, key) =>
                key.canonicalWorktreePath === keyB.canonicalWorktreePath
                    ? groupsBySession['session-b']
                    : groupsBySession['session-a'],
    ...
    t.after(() => viewer.dispose());
    const openTarget = sessionId => ({
        projectId: 'project', provider: 'codex', sessionId,
        workspaceName: 'Workspace', interactionId: 'input-1',
        expectedRevision: 'r1', displayName: sessionId,
        duplicateDisplayName: false,
    });
    const lastChanges = () => panel.postedMessages
        .filter(message => message.type === 'conversation-viewer-changes')
        .at(-1)?.changes;
    const waitForChanges = async predicate => {
        for (let attempt = 0; attempt < 50; attempt += 1) {
            const state = lastChanges();
    ...
    await viewer.open(openTarget('session-a'));
    const stateA = await waitForChanges(state =>
        state.members[0]?.memberId === 'member-a');
    assert.ok(stateA, 'session A publishes its changes');
    assert.deepEqual(stateA.detail.items.map(item => item.path),
        ['changed.ts']);

    await viewer.open(openTarget('session-b'));
    const stateB = await waitForChanges(state =>
        state.members[0]?.memberId === 'member-b');
    assert.ok(stateB,
        'switching sessions republishes the changes for the new session');
    assert.deepEqual(stateB.detail.items.map(item => item.path).sort(),
        ['also-changed.ts', 'changed.ts']);
    assert.notEqual(lastChanges()?.members[0]?.memberId, 'member-a',
        'the panel never settles on the previous session');
});
