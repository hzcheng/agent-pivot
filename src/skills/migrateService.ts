...
import type { SkillRecord, SkillSourceDir } from './types';
...
/**
 * One-shot migration of every enabled user-scope skill living in the three
 * brand agent roots into the central store (`~/.skills`). The highest
 * priority copy (kimi > claude > codex) wins; all other real-directory
 * copies are parked reversibly under their root's `.disabled/`. No agent
 * links are created — users enable agents per card afterwards.
 */
export function migrateUserSkillsToCentral(records: SkillRecord[], homeDir: string): SkillMigrationReport {
    ...
    const centralRoot = getCentralSkillsRoot(homeDir, 'user');
    for (const [name, copies] of [...groups.entries()].sort(([a], [b]) => a.localeCompare(b))) {
        ...
        const result = centralizeSkill(winner, losers, homeDir, undefined, { linkBack: false });
        if (!result.ok) {
            report.ok = false;
