...
function runWorkspaceProjectionV4Checks() {
    ...
    assert.deepStrictEqual(
        ...
        'cards sort by stable opening time, then navigation identity'
    );
...
async function runOpenWorkspaceClientAndControllerChecks() {
    ...
    const savedCardId = identityDashboard.getCards().find(card => card.kind === 'current').id;
    assert.strictEqual(
        identityDashboard.getCards().find(card => card.kind === 'current').showSaveAction,
        true,
    ...
    assert.strictEqual(posted[0].version, 4);
    assert.strictEqual(
        posted[0].presentation.projectionRevision,
        posted[0].projectionRevision
    );
    assert.strictEqual(posted[0].currentWindowRowCount, 1);
    assert.strictEqual(posted[0].navigationWindowRowCount, 2);
    assert.strictEqual(posted[0].windowRowCount, 3);
    assert.strictEqual(posted[0].currentDetailCount, 1);
    ...
    assert.strictEqual(
        dashboard.getCards().find(card => card.kind === 'current').attentionCount,
        1
    );
}
