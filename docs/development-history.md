...
This file archives the unpublished development history that preceded the
first public Agent Pivot release (1.0.0). None of the versions below were
published to the VS Code Marketplace; they belong to internal identities
used before the Agent Pivot rename. This archive moved out of
`CHANGELOG.md` so the published changelog only covers shipped Agent Pivot
versions.
