...
| Stop Session through UI | NOT RUN | New flow (2026-08): the tmux active-row card action terminates the AI task (project layout kills only that session's managed window; session layout kills the managed session); the attached viewer terminal closes itself; the row leaves Active and the session stays resumable in history. Contract coverage passed in `npm run test:tmux` and `npm run test:safety`; the installed UI flow awaits manual exercise. |
| Detach through the row context menu | NOT RUN | Detach moved from the card action to the context menu (2026-08) and is enabled only while the session has an attach terminal; the menu also offers Stop Session for tmux rows. |
