...
区域空间回收（折叠不留白）：`OPEN WINDOWS` 折叠时收缩为标题栏并固定在面板底部，`CURRENT WINDOW` 解除 50% 高度上限——sidebar 中展开的详细卡片填满释放的全部空间，宽面板中保持内容高度但不设上限；`CURRENT WINDOW` 详细卡片收起时区域回到内容高度，`OPEN WINDOWS` 向上压实。两个区域之间的分隔条只在双方都展开时出现；拖拽过的比例在任一区域折叠期间挂起（不丢弃），双方恢复展开时自动回放。
