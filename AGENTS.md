...
- Dashboard webview HTML/icons: `src/webview/webviewContent.ts`,
  `src/webview/webviewIcons.ts`; webview script sources: `src/webview/*.js`
  (the byte-identical `media/*.js` copies are build outputs regenerated by
  `npm run test-compile`, never edit or commit them); webview styles:
  `media/`
