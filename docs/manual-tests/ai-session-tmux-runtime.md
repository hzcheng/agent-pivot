...
| Stop Session through UI | PASS | Executed 2026-08-03 on the installed Agent Pivot 1.0.2 build (operator-confirmed): the tmux active-row card action terminated the running AI task, the attached viewer terminal closed itself, the row left Active, the session stayed resumable in history, and sibling sessions were unaffected. Contract coverage passed in `npm run test:tmux` and `npm run test:safety`. |
